...
class ValidationRefreshConfig(BaseModel):
    enabled: bool = False
    max_attempts: int = Field(default=0, alias="maxAttempts")
    interval_ms: int = Field(default=150, alias="intervalMs")

    model_config = {"populate_by_name": True}


class ValidationExtractionConfig(BaseModel):
    extractor_profile: Optional[str] = Field(default=None, alias="extractorProfile")
    refresh: ValidationRefreshConfig = Field(default_factory=ValidationRefreshConfig)
...
class ValidationElementTypeCatalog(BaseModel):
    ...
    extraction: ValidationExtractionConfig = Field(default_factory=ValidationExtractionConfig)
...
class ValidationElementSnapshot(BaseModel):
    ...
    selected_option: Optional[str] = Field(default=None, alias="selectedOption")
    option_count: Optional[int] = Field(default=None, alias="optionCount")
    dropdown_options: list[str] = Field(default_factory=list, alias="dropdownOptions")
